...
async function createBooking(input: { courseId: string; date: string; time: string; players: number; guest?: { firstName: string; lastName: string; email: string; phone: string } }, opts?: { idempotencyKey?: string }): Promise<Booking> { throw new Error('chronogolf adapter not implemented') }
async function cancelBooking(bookingId: string): Promise<{ status: 'cancelled' | 'not_found'; booking?: Booking }> { throw new Error('chronogolf adapter not implemented') }
