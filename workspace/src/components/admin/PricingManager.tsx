...
export function PricingManager({ courseId, courseName }: PricingManagerProps) {
  ...
  const handleTimeBandSubmit = (formData: FormData) => {
    handleAsyncError(async () => {
      ...
      // Validar máximo 3 bandas activas
      const isActive = formData.get('active') === 'on';
      if (isActive) {
        const activeCount = timeBands.filter(tb => 
          tb.active && 
          (!editingTimeBand || tb.id !== editingTimeBand.id)
        ).length;
        
        if (activeCount >= 3) {
           throw new ValidationError('Solo se permiten máximo 3 bandas horarias activas simultáneamente.');
        }
      }

      // Validar duplicado de etiqueta (debe ser única)
      const isLabelDuplicate = timeBands.some(tb =>
        (!editingTimeBand || tb.id !== editingTimeBand.id) &&
        tb.label.trim().toLowerCase() === label.trim().toLowerCase()
      );
      if (isLabelDuplicate) {
        throw new ValidationError('Ya existe una banda horaria con esta etiqueta.');
      }

      // Validar solapamientos: no se pueden cruzar horarios (solo para bandas activas)
      // Si la banda está inactiva, permitimos solapamiento (opcional, pero seguro para evitar conflictos lógicos)
      if (isActive) {
        const overlaps = timeBands.some(tb => {
          if (!tb.active) return false; // Ignorar inactivas para conflicto de horarios
        ...
        if (overlaps) {
          throw new ValidationError('Los horarios se cruzan con otra banda activa existente. Ajuste el rango.');
        }
      }
                ...
                <Dialog open={timeBandDialog} onOpenChange={setTimeBandDialog}>
                  ...
                  <DialogContent>
                    ...
                    <form onSubmit={(e) => {
                      ...
                      formData.append('active', timeBandForm.active ? 'on' : 'off');
